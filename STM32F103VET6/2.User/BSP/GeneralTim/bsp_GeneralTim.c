/* 引用头文件 ------------------------------------------------------------------*/  
#include "./GeneralTim/bsp_GeneralTim.h"  //引用GeneralTim配置头文件
...
/* 程序开始 ----------------------------------------------------------------------*/
 ...
 /**
  * @brief  ：GeneralTim_GPIO_Config(void);	//通用定时器3输出比较通道 GPIO引脚配置函数
  * @param  ：参数说明
  * @arg    ：参数里面可选择参量列举
  * @note   ：注释
  * @retval ：返回值说明
...
{
	uint16_t	ARR	=	72000/frequency-1;	//设定自动重装载寄存器周期的数值，freq单位为KHz，一般直接使用72000
	
	// 开启定时器时钟,即内部时钟CK_INT=72M
	GENERAL_TIM_APBxClock_FUN(GENERAL_TIM_CLK,ENABLE);	//示例打开的是通用定时器3的使能时钟
