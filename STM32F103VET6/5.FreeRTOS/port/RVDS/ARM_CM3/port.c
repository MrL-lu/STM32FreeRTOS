...
void xPortSysTickHandler( void )    // SysTick 中断服务函数
{
	...
	vPortRaiseBASEPRI();    // 关中断
	{
		...
		if( xTaskIncrementTick() != pdFALSE )   // 更新系统时基
		{
 ...
 */
    ...
    void vPortSetupTimerInterrupt( void )   // 初始化 SysTick
	{
		...
		portNVIC_SYSTICK_LOAD_REG = ( configSYSTICK_CLOCK_HZ / configTICK_RATE_HZ ) - 1UL;  // 设置重装载寄存器的值
